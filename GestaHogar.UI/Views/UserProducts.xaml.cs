...
public partial class UserProducts : ContentPage
{
    ...
    private async void OnEditProductClicked(object sender, EventArgs e)
    {
        ...
        {
            await Navigation.PushAsync(new FormUserProduct(product, false));
        }
    }
    ...
    private async void OnDeleteProductClicked(object sender, EventArgs e)
    {
        if (sender is Button btn && btn.CommandParameter is UserProductDto product)
        {
            ...
            if (confirm)
            {
                var response = await GHHttpClient.Client.DeleteAsync(GHHttpClient.DeleteUserProductUri((int)product.ProductId!));
                if (response.IsSuccessStatusCode)
                {
    ...
    private async void OnUpdateAllClicked(object sender, EventArgs e)
    {
        ...
        bool confirm = await DisplayAlert("Actualizar productos",
            "¿Seguro que desea actualizar todos los productos?"+
            $"Esta operación establecerá el stock actual de todos a su stock normal", "Sí", "No");

        if (!confirm) return;

        var response = await GHHttpClient.Client.PutAsync(GHHttpClient.PutUpdateAllUserProductsUri, null);
        if (response.IsSuccessStatusCode)
        {
    ...
    private async void OnUpdateProductClicked(object sender, EventArgs e)
    {
        ...
        {
            bool confirm = await DisplayAlert("Actualizar producto",
                $"¿Seguro que desea actualizar {userProduct.ProductName}? " +
                $"Esta operación establecerá su stock actual a su stock normal", "Sí", "No");

            if (!confirm) return;

            var response = await GHHttpClient.Client.PutAsync(GHHttpClient.PutUpdateUserProduct((int)userProduct.ProductId!), null);

            if (response.IsSuccessStatusCode)
            {
                await DisplayAlert("Éxito", $"Producto {userProduct.ProductName} actualizado.", "OK");
                userProduct.CurrentStock = userProduct.NormalStock;
            }
            else
            {
                await DisplayAlert("Error", $"No se pudo actualizar el producto {userProduct.ProductName}.", "OK");
            }
        }
    }
}